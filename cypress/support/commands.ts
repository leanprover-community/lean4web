...
const containsAllFn: Cypress.ContainsAllFn = (subject, selector, contents, options) => {
  ...
  return contents.reduce((chain, content) => {
    return chain.contains(selector, content, options)
  }, cy.wrap(subject, options))
}

Cypress.Commands.add('containsAll', {prevSubject: ['optional', 'element']}, containsAllFn)
